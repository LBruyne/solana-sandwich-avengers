"""
Phase 3: Attacker Filter — Identify Intentional Sandwich Attackers
================================================================
...
Usage:
    python 3_attacker_filter.py --start-epoch 946 --end-epoch 960
...
def classify_signers(sf, ps, n_min, wr_min, slip_min, fg100_min,
    ...
    n_dropped = len(jito_candidates) - len(jito_attackers)
    if jito_candidates:
        print(f"  Jito Bot: {len(jito_candidates)} same-bundle candidates -> "
              f"{len(jito_attackers)} retained "
...
def print_report(sf, tier_series, ps, token_prices,
    ...
    print(f"  {'Bot (intentional)':<20} {len(bot_attackers):>10,} "
          f"{len(bot_attackers)/total_signers*100:>6.2f}% {bot_sw:>12,} "
          f"{bot_sw/total_sw*100:>6.1f}% {bot_sol:>12,.1f} ${bot_usd:>13,.0f}")
    print(f"  {'Non-bot':<20} {total_signers-len(bot_attackers):>10,} "
          f"{(total_signers-len(bot_attackers))/total_signers*100:>6.2f}% {non_bot_sw:>12,} "
    ...
    # Top 20 bot signers
    print(f"\n  --- Top 20 Attackers (by sandwich count) ---")
...
def main():
    ...
    if category == "multi_split" and variant != "all":
        out_dir = f"data/3_attacker_filter/{category}/{variant}"
    else:
        out_dir = f"data/3_attacker_filter/{category}"
